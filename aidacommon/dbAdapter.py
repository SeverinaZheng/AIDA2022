...
class DBC(metaclass=ABCMeta):
    ...
    def _Schedule(self,iter_func,cond_func,test_func,*args,**kwargs):
        ...
        while not cond_func(self, *args, **kwargs):
            ...
            condition.acquire(blocking=True);
            logging.info("before scheduling");
            self._schMgr.schedule_GPU(condition);
            condition.wait();
            if(self._schMgr.in_GPU(condition)):
                use_gpu = True;
                logging.info("run on gpu");
                #self._schMgr.start_working_success();
                result = all_to_gpu();
                condition.release();
            else:
                ...
                self._schMgr.finish_CPU(condition);
                condition.release();

        if(not use_gpu):
            result = test_func(TensorWrap_CPU(self), *args, **kwargs);
        else:
            self._schMgr.finish_GPU(condition);
        end_time = time.time()
        result = result + "time: " + str(end_time - start_time)
        return result;
